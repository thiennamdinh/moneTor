...
\item No additional party needed to operate the MoneTor system (i.e. ledgers
  ...
  known by an existing relay. \flo{We can be more precise, saying by an existing *middle* relay? Exit or guard relays know already more about users than an inter or ledger}
...
connections have been perfectly anonymized. \flo{Careful with adjectives: perfect anonymity does not exist in real-world (i.e., there is always some leakage)}
...
operations. \flo{We probably need to introduce to the reader here why we thought a nanopayment channel was needed. Indeed, the Bolt paper claims that their micropayment can be used to do Tor bandwidth prioritization, while we strongly disagree with.}
...
channel is trivially linked to each other. However, each nanopayment channel is
...
order of millions per second and each payment can be relayed with a single
256 bit message (one cell in Tor's case).
...
\flo{We definitely need to prove that nanopayment channels are unlinkable.}
