...
latency is unchanged: there is no reason for the first byte to come faster for premium users since our priority mechanism only change the size of the control flow window (i.e., how much cells the circuit is allowed to carry at any time, see again Section~\ref{subsub:prioritized} for details). Such approach would keep the network responsive for nonpremium and low-bandwidth usage, while still offering an upward benefit for premium users when loading web pages, as shown in Figure~\ref{fig:modifier}.
