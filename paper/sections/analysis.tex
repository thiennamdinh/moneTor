...
strategy. \op{``should ideally'': is it really ideally? My understanding is that this point motivated the creation of payment channels at circuit creation time, that is, before any real traffic happens.}
...
\op{Why do you have 2 sets of curves in (a) and (c)? For the two file sizes? This should probably be added.}
...
\includegraphics[trim={0 3cm 0 3cm}, clip, width=1.0\textwidth]{images/overhead_downloadtime.pdf}
		...
		\label{fig:stats_c}
	\end{subfigure}
	\caption{Global Overhead}
	\label{fig:overhead}
\end{figure*}

Our findings indicate that even the worst case scenario our system incurs
statistically negligible overhead at these scales. Note that regarding bandwidth overhead, a small fraction ($< 1\%$) of the network traffic is attributable to moneTor payment messages, with a payment rate of 1 payment (1 cell) every 1000 data cells exchanged in either direction. In comparison, Tor's control-flow mechanism induces more than 10 times this overhead cost. Nothing prevent us to increase the payment rate to achieve more fairness if needed, as far as the total overhead induced from control cells is kept under an acceptable fraction of the overall data bandwidth.
...
protocol. The results shown here are collected from the worst case 100 relays, 1000 Clients experiment featuring
100\% premium clients. To highlight the effects of native latency in the Tor
...
\label{fig:payments_close}
	\end{subfigure}
	\caption{Protocol Execution Time on a simulation featuring 100 relays, 2 authorities, 1 ledger authority, 10 intermediaries and 1000 Tor clients scaled down from the public consensus file `2018-02-03-00-00-00-consensus'.}
\label{fig:latencymeasurements}
\end{figure*}


Figure~\ref{fig:ttfp} illustrates time to first payments, our most
revealing latency metric. This measure includes
the overhead in channel establishment when we do not have available preemptive
channels. In the best case scenario, when the 3 payment channels are correctly pre-built for the circuit, this measure is equivalent to one round trip time to each of the relays. Comparing this Figure~\ref{fig:ttfp} to Figure~\ref{fig:payments_establish}, we observe the effectiveness of preemptive channel building.
...
little loss in overall network performance, and confirming our overhead experiment.
...
\label{fig:modifier_pr25_all}
	\end{subfigure}
	\caption{Prioritization Benefit on simulations featuring 250 relays, 2 authorities, 1 ledger authority, 25 intermediaries and 2500 Tor clients scaled down from the public consensus file `2018-02-03-00-00-00-consensus'. Figures (a), (b) and (c) show results for 50\% of the clients being premium users. Figure (d), (e) and (f) show results for 25\% of the clients being premium users.}
\end{figure*}
