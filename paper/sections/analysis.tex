...
baseline trial with 0\% premium clients. The payment protocols run but the control-flow and EWMA stay unchanged to only compare the CPU cost of the payment scheme (that is, we do not apply any priority function). Since our algorithms makes use of some concurrent crytpographic
...
prioritization which, since a poor choice of parameters can lead to an overall decrease of throughput. One of the take-out message of our research and results, is that we believe the prioritization scheme to be more complex to get right and fulfill its goal in practice than the design of the anonymous payment layer itself.
