...
simulated crytographic operations such as zpk proofs and commitments in the nanopayment creation and close
procedures using CPU delays. Furthermore, those delays are implemented in account for Shadow CPU delay design: if the Tor process is considered to run over a multicore computer, the costly operations are simulated over a dedicated core. If the Tor process is considered to run over a singlecore computer, then the costly operations are simulated by an appropriate number of dummy AES encryptions in a thread concurrently to the Tor process main thread. Those AES encryptions are captured by Shadow and replaced by CPU delays on the virtual CPU, affecting Tor's main thread.
