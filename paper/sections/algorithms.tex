...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \Procedure{Client}{}
      ...
      \State{$B^n_C = B_C + (\delta_C * n)$}
      \State{$nrt_C \gets $Intermediary.Blindsig$(ver, refund || nT || nwpk_C || B^n_C)$}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \Procedure{Relay}{}
      \State{$ver \gets $Intermediary.Receive$()$}
      \State{$B^n_R = B_R + (\delta_R * n)$}
      \State{$nrt_R \gets $Intermediary.Blindsig$(ver, refund || nT || nwpk_R || B^n_R)$}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \Procedure{EndUser}{}
      ...
      \State{$B^k_E = B_E + (\delta_E * k)$}
      \State{$rt'_E \gets $Intermediary.Blindsig$(ver, refund || wpk'_E || B^k_E)$}
