...
queue, which in turn is positively correlated with effective bandwidth. AlSabah \textit{et al.}~\cite{pets2011-defenestrator} already played with control flow windows in attempt to improve performance. Our objective slightly differ: we want to improve the performance of a subset of users (premium users), at the cost of decreasing the performance for unpaid bandwidth.
We implement our prioritization scheme by readjusting the windows according to the
following formula.
\begin{equation}
