...
In our payment design, we follow the Bitcoin paradigm in
which all users maintain full and exclusive control of their monetary wealth
through use of public key cryptography~\cite{nakamoto2008bitcoin}. However, unlike
...
database~\cite{crosby2009efficient}. To maximize availability,
this ledger may also be distributed across several authorities.
...
micropayment operations. The idea here is to relax payments unlinkability offered by Bolt because we do not need each payment to be unlinkable in Tor: we do not care about unlinkability of incremental payments inside a same circuit to a same relay but we do care about unlikability of payments to different circuits and to different relays.
...
payment protocol. A more formal treatment of the steps are provided
in Appendix~\ref{chap:algorithms} with algorithmic details. Security considerations are detailed in following Section~\ref{subsec:paysecurity} and formally described in Appendix~\ref{sec:proof}.
...
\textbf{Nano-Close} After some number of payments $k < n$ has transpired or when $C$ desires to close the Tor circuit, $C$ and $R$ will both wish
...
The zero-knowledge setup also allows an elegant way to anonymously handle the
tax collection policy described in Section~\ref{sec:economic}. Thus far, we have
...
definitions, theorems, and proofs are provided in Appendix~\ref{sec:proof}.

Observing the cash-out transactions on the ledger does not reveal any
information regarding the source of the payment. Moreover, any communication in
...
The unmodified construction exposes a subtle passive attack by $I$. By examining
