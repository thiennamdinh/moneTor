...
\medskip \noindent\textbf{moneTor.}
The moneTor design uses the existing anonymous micropayment structure to build \emph{locally transparent nanopayments}.
...
This design ensures that any attempts to prematurely close either channel---which requires revealing $hc_0$ to the network---would implicitly close the other channel as well.
