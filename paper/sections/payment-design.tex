...
%particular circuit.
However, given an adequately privacy-preserving choice of an
on-ledger transaction protocol such as Zerocash, even this result would be
meaningless for $R$~\cite{sasson2014zerocash} (that is, $R$ can only link the ledger 
interaction to its circuit, but not the client's identity protected by the built-in ledger 
privacy).
