...
\subsubsection{Unlinkability of Micropayment}
...
anonymity set size for payment events. This number could be recalculated based on the activity of premium users: assuming we want an anonymity set size of $N$, then the maximum allowed Intermediaries in the network could be defined by $\frac{\mathbb{E}[W]}{N}$ with W a random variable holding the number of payment events between any $t, t+1$. Moreover, clients 
...
nanopayment channel, virtually increasing the anonymity set size and 
requiring intermediaries to coerce to make use of any apriori 
