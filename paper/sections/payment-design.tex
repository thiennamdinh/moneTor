...
database~\cite{crosby2009efficient}. For availability concerns, nothing prevents this ledger to be distributed across several authorities.
...
handful of concurrently active circuits,\footnote{For instance, the popular Tor
  Browser user application typically do not share circuits with streams targeting a different destination address unless those streams come from the same SOCKS connection}  each of
...
between the client and his relays is protect by the various Tor circuits built
